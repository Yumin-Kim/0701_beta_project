...
AJAXRequestMethod({ method: "GET", requestURL: `${serverURL}/ece1000` })
    .then(async (response) => {
        ...
        const { data: mininghistoryList } = await AJAXRequestMethod({ method: "GET", requestURL: `${serverURL}/ece6000/ece6100_beta?member=${member}` })
        if (mininghistoryList.length !== 0) {
            let innerHTMLText = mininghistoryList.map((historyList) => {
                ...
                let historyUlTag = historyList.dayminingData.map((resourceData) => {
                    let spAmount;
                    let resoureInner = resourceInnerHTML({ cateogoryName: selectCodeNameTpCodeTable({ data, codeName: resourceData.resource }), amount: resourceData.amount })
                    let innerHTML_1 = textInnerHTML({ minerCount: resourceData.minerCount, tileCount: resourceData.tileCount, spAmount: null })
